...
const VillageCard = ({room, index}) => {
    return (
         <Link to={"/stays/" + room._id} onClick={()=> scrollTo(0,0)} key={room._id} 
         ...
         <div className='p-4 pt-5'>
            ...
            <div className='flex items-center justify-between mt-1'>
                 <img className='flex items-center gap-1' src={assets.locationIcon} />
                 <span>{room.hotel.address} </span>
            </div>
            <div className='mt-4'>
                <p className='text-xl text-gray-800'> <span> ${room.pricePerNight}</span>/night </p>
                <button className='mt-2 px-4 py-2 text-sm font-medium border border-gray-300 rounded hover:bg-gray-50 transition-all cursor-pointer'>
                    Book now 
                </button>
            </div>
         </div>
