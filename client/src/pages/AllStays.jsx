...
<div>
            ...
            <h1 className='font-playfair text-4xl md:text-[40px]'>All Stays</h1>
 ...
 {roomsDummyData.map((room) => (
    ...
    <div key={room._id} className=' flex flex-col  md:flex-row items-start py-10 gap-6 border-b  border-gray-300 last:pb-30 last:border-0'>   
        <img onClick={()=> {navigate(`/stays/${room._id}`); scrollTo(0,0)}}
         src={room.images[0]} alt="hotel-img" title='View Stay Details'
        className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'  />
        <div className='md:w-1/2 flex flex-col gap-2'>
            <p className='text-gray-500'>{room.hotel.city}</p>
            <p onClick={()=> {navigate(`/stays/${room._id}`); scrollTo(0,0)}} className='text-gray-800 text-3xl font-playfair cursor-pointer'> {room.hotel.name} </p>
